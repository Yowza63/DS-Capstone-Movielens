...
The model will be measured on a RMSE (Root Mean Squared Error) of the predicted outcomes (ratings) and the actual results for the validation dataset. The goal is to have the smallest RMSE as possible. The benchmark given for the project is 0.86490
...
There appear to be clear user effects.
Most user's rate < 100 movies and those users who rate more movies tend to have higher ratings.
...
Are there specific movie effects in the data?  For example, do famous movies (as defined by those with many ratings) have overall higher ratings?
Are there anomolies in the movie specific data?
...
# Explore user's behavior around when they've rated the movies
...
## What is the timeline of user's activity? 
## Do user's who've been rating longer have more movies rated?

The finding is interesting. While most users logged their ratings over a short period of time, it 
doesn't seem to impact their rating behavior.
...
## Matrix Completion (aka collaborative filtering)
A popular technique to solve the recommender system problem is the matrix factorization method. The idea is to approximate the whole rating matrix [ ] by the product of two matrices of lower dimensions, [ ], such that
[ ] see https://statr.me/2016/07/recommender-system-using-parallel-matrix-factorization/
...
#TODO find out how to show the mathmatical formulas
The recosystem is an R wrapper of the LIBMF library that inherits most of its features. The documentation can be found: https://www.rdocumentation.org/packages/recosystem/versions/0.3


"A tuning parameter (λ), sometimes called a penalty parameter, controls the strength of the penalty term in ridge regression and lasso regression. It is basically the amount of shrinkage, where data values are shrunk towards a central point, like the mean." - https://www.statisticshowto.com/tuning-parameter/
...
recosystem uses cross-validation to find the tuning parameters, https://rdrr.io/cran/recosystem/man/tune.html
...
The data is challenging because user's rate so few movies. Ideally, you'd have more information about users to create profiles. For Netflix, I assume they have massive data on watching patterns that could be leveraged. Do user's watch certain movies to the end, but others they start and never finish?
...
Helped me master R or at least get better
There is usually more than one way to get something done in R and it is easy to get sidetracked learning a new library
RStudio has amazing features to make work easier. Monitoring the environment and using the View feature which can be done by typing View(edx) for example to see the edx dataset in a tabular spreadsheet format or just click on the spreadsheet icon from within the Global Environment tab
Sometimes the only way to get comfortable with the code is to extract a subset into a .csv and mannualy check the match using Excel. I hope someday to be confident enough in R to be able to do this validation within the R environment.
